...
import br.ufla.dcc.PingPong.MXMac.MXMacState.MXMacActionType;
...
import static br.ufla.dcc.PingPong.MXMac.MXMacConstants.TUNE_INTO_NEIGHBOR_CHANNEL;
import static br.ufla.dcc.PingPong.MXMac.MXMacConstants.RESTORE_DEFAULT_CHANNEL;
import static br.ufla.dcc.PingPong.MXMac.MXMacConstants.KEEP_CHANNEL;
...
public class MXMacStateMachine  {
	 ...
	 * Default constructor 
	 */
    public MXMacStateMachine(Address address, MXMacState xmacState, MXMacConfiguration xmacConfig) {
    ...
    private boolean setNewState (MXMacStateType newState, double delay, MXMacActionType action, int radioMode){
		xState.setAction(action);
		xState.setRadioChannelMode(radioMode);
    ...
    public boolean changeState(MXMacEventType event){
    	
    	boolean changeState = false;
    	xState.setAction(MXMacActionType.CONTINUE);
    		...
    		case TIME_OUT:
    			if (xState.isDataPending()) { 
    				...
    				if(xState.getDataPkt().decRetryCount() > 0){
    					...
    					changeState = setNewState(MXMacStateType.CS_START, xConf.getStepsCS(), MXMacActionType.ASK_CHANNEL, TUNE_INTO_NEIGHBOR_CHANNEL);
    					if(debug) System.out.println("Machine: " + address.getId()+ " State = SLEEP, Event = TIME_OUT com dataPending" );
    				} else {
    					changeState = setNewState(MXMacStateType.CS, xConf.getStepsCS(), MXMacActionType.TURN_ON, RESTORE_DEFAULT_CHANNEL);
    				...
    				changeState = setNewState(MXMacStateType.WAITING_DATA, xConf.stepsDelayRx(PacketType.DATA), MXMacActionType.TURN_ON, KEEP_CHANNEL);
    	...
    	case CS:
    		...
    		case TIME_OUT:
    			...
    			changeState = goToSleep(MXMacActionType.TURN_OFF);
    		...
    		case RTS_RECEIVED:
    			...
    			changeState = changeStateRTSreceived(MXMacStateType.CS);
    		break;
    	...
    	case CS_START:
    		...
    		case TIME_OUT:
    			if (xState.isChannelBusy()) {
    				...
    				changeState = setNewState(MXMacStateType.BO_START, xConf.getStepsBOstart(), MXMacActionType.TURN_OFF, KEEP_CHANNEL);
    				...
    				changeState = setNewState(MXMacStateType.SENDING_RTS, xConf.stepsDelayTx(PacketType.RTS), MXMacActionType.START_RTS, KEEP_CHANNEL);
    		 ...
    		 * responda como se fosse um CS e tente iniciar envio mais tarde  */
    			changeState = changeStateRTSreceived(MXMacStateType.CS_START);
       		break;
    	...
    	case CS_END:
    		...
    		case TIME_OUT:
    			...
    			xState.setAction(MXMacActionType.ASK_CHANNEL);
    		...
    		case CHANNEL_FREE:
    			...
    			changeState = goToSleep(MXMacActionType.TURN_OFF_CS_END);
    		...
    		case RTS_RECEIVED:
        		 ...
        		 * responda como se fosse um CS e tente iniciar envio mais tarde  */
    			changeState = changeStateRTSreceived(MXMacStateType.CS_END);
    		break;
        ...
        case SENDING_RTS:          
    		...
    		case MSG_SENT:
    			...
    			changeState = setNewState(MXMacStateType.WAITING_CTS, xConf.stepsDelayRx(PacketType.CTS), MXMacActionType.TURN_ON, KEEP_CHANNEL);
        ...
        case SENDING_DATA: 
    		...
    		case MSG_SENT:
    			...
    			if (xState.getDataPkt().isAckRequested()){
    				changeState = setNewState(MXMacStateType.WAITING_ACK, xConf.stepsDelayRx(PacketType.ACK), MXMacActionType.TURN_ON, KEEP_CHANNEL);
    			}
    			else {
    				xState.setDataPending(false);
    				changeState = setNewState(MXMacStateType.CS_END, xConf.getStepsCS(), MXMacActionType.TURN_ON, RESTORE_DEFAULT_CHANNEL);
    			}
        ...
        case SENDING_ACK: 
    		...
    		case MSG_SENT:
    			...
    			changeState = goToSleep(MXMacActionType.MSG_UP);
    	...
    	case WAITING_CTS: 
    		...
    		case TIME_OUT:
    			...
    			if (rtsSeqNum > 0) {
    				...
    				changeState = setNewState(MXMacStateType.SENDING_RTS, xConf.stepsDelayTx(PacketType.RTS), MXMacActionType.MSG_DOWN, KEEP_CHANNEL);
    				...
    				if (xState.getRtsPkt().getReceiver() != NodeId.ALLNODES) {
    					...
    					changeState = setNewState(MXMacStateType.SENDING_DATA, xConf.stepsDelayTx(PacketType.DATA), MXMacActionType.MSG_DOWN, KEEP_CHANNEL);
        ...
        case WAITING_DATA: 
    		...
    		case DATA_RECEIVED:
    			...
    			if (xState.getRecPkt().isAckRequested()){
    				changeState = setNewState(MXMacStateType.SENDING_ACK, xConf.stepsDelayTx(PacketType.ACK), MXMacActionType.MSG_DOWN, KEEP_CHANNEL);
    			}
    			else {
    				changeState = setNewState(MXMacStateType.CS_END, xConf.getStepsCS(), MXMacActionType.MSG_UP, RESTORE_DEFAULT_CHANNEL);
    			}
    ...
    private boolean goToSleep(MXMacActionType action) {
    	//double operationSteps = SimulationManager.getInstance().getCurrentTime() - stepsFromLastSleep;
    	return setNewState(MXMacStateType.SLEEP, xConf.getStepsSleep(), action, KEEP_CHANNEL);
    }
    
    /** Função para decidir o que fazer quando receber um RTS.     
     */ 
    public boolean changeStateRTSreceived(MXMacStateType oldState) {
    	boolean changeState;
		...
		if (xState.getRecPkt().getReceiver().equals(this.address.getId())){
	        ...
	        changeState = setNewState(MXMacStateType.SENDING_CTS, xConf.stepsDelayTx(PacketType.CTS), MXMacActionType.MSG_DOWN, KEEP_CHANNEL);
	     ...
	     * Espera = (número de RTS que faltam) * (intervalo entre envios de RTS) */
	    	...
	    	changeState = setNewState(MXMacStateType.SLEEP, delay, MXMacActionType.TURN_OFF, KEEP_CHANNEL);
    ...
    public void changeStateBootNode() {
    	...
    	xState.setState(MXMacStateType.SLEEP, delay);
    }
