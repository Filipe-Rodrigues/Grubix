 ...
 * Use two, tree or more nodes to test three different modes.
 * 
 *  - two:   node 1 sends to node 2 and before node 1 is finished, node 2 sends to node 1.
 *  - three: node 1 and 3 send to node 2 at the same time.
 *  - more:  node n sends to node n+1 at the same time.
 * 
 * @author Dirk Held
 */
public class AppABCtest extends ApplicationLayer {
	...
	private static final Logger LOGGER = Logger.getLogger(AppABCtest.class.getName());
	...
	public final void lowerSAP(Packet packet) {
		...
		if (packet instanceof ApplicationPacket) {
			...
			if (mode == 1) {
				if (inCount == 1) {  // wait for 2 broadcasts in total.
					appState = ApplicationState.DONE;
				}
			} else if (mode == 2) {
				if (forThisNode) {  // wait for one packet.
					appState = ApplicationState.DONE;
				}
			} else if (mode == 3) {
				if (inCount == 1) { // wait for two packets in total
					appState = ApplicationState.DONE;
				}
			} else if (mode == 4) {
				if (forThisNode) {
	...
	protected void processEvent(StartSimulation start) {
		int me = id.asInt(), other = me + 1;
		double t = getConfig().getPropagationDelay() * 0.5;  // half propagation delay
		//double t = Configuration.getSimulationSteps(0.00001);  // wait SIFS
		...
		if (mode == 1) {
			...
			if (me % 2 != 0) {
				if (other > nodeCount) {
					appState = ApplicationState.DONE;
					// ups, odd node count in non-simple mode, thus no partner
					// for this node.
				} else {
					sendTo(other);
				}
			} else {
				appState = ApplicationState.WAITING; // the receiver does
														// nothing
			}
		}
	}
