 ...
 *  Para isto a função setBPS coloca a velocidade em  1,000,000,000 bits por segundo.
 ...
 *  
 */
public class PhysicalSimple extends PhysicalLayer {
	...
	/** Tamanho adicional do pacote da PHY, será somado ao tamanho do pacote vindo da MAC. Por 
	 * enquanto será deixado em zero */
	...
	private static final Logger LOGGER = Logger.getLogger(PhysicalSimple.class.getName());
 	
	/** Objeto para realizar a contagem estatística */
 	ToolsStatisticsSimulation statistics = ToolsStatisticsSimulation.getInstance();
 	
	public PhysicalSimple() {
		super();
	}
	...
	public void upperSAP(Packet packet) {
        ...
        if (!(packet instanceof MACPacket)) {
            debug.print("[!!!AVISO] Mensagem desconhecida: "+packet.getClass().getName(), sender);
            return;
        }
        ...
        PhysicalPacket nextPacket = new PhysicalPacket(sender, pkt);
	...
	public void lowerSAP(Packet packet) {
		...
		/* Trata apenas o início da recepção de mensagens, correspondente ao SFD 
		 (Start of Frame Delimiter) de um rádio real. O processamento após o final da 
		 recepção é feito pelo WUC de rádio */
		if (packet.isTerminal()){
			debug.print("[!!!AVISO] Pacote é terminal", sender);
			return;
		} 
		
		if (!packet.isValid()) {
			if (dropInvalidPackets) {
				//debug.print("[!!!AVISO] Descartando mensagem inválida, destino: "+ packet.getReceiver(), sender);
				return;
			} else {
				debug.print("[!!!AVISO] Recebendo mensagem inválida", sender);
			}
		}
		...
		/* Se o tempo final para a transmissão atual for maior que a vigente, então irá atualizar o tempo 
		 final e criar evento para marcar o tempo em que o rádio ficará recebendo sinais da mensagem 
		 que está chegando */
		...
		/* Todas as verificações feitas anteriormente são para verificar atividade no canal de comunicação
		 A seguir começam as verificações para a recepção de mensagem pelo rádio do nó sensor. Caso o rádio
		 não esteja em LISTENING ou RECEIVING, irá ignorar a mensagem */	
		if((phyRadioState.getRadioState() != RadioState.LISTENING) && 
		...
		if(msgColision) {
			...
			sendEventUp(new ColisionDetectEvent(sender, incomingPacket.getEnclosedPacket()));
		} else {
	...
	public void processWakeUpCall(WakeUpCall wuc) throws LayerException {
		...
		if (wuc instanceof CrossLayerEvent) {
			/* Evento que vem de node.AirModule:processTransmissionEndOutgoing. Que é enviado quando 
			 * acaba de enviar uma mensagem. A classe node.AirModule:processTransmissionEndOutgoing usa 
			 * a função sendEventUp() para enviar um evento para a camada física */
		...
		// Quando uma mensagem completamente enviada pela AIR
			...
			if (currentTimeSteps() >= lastReceptionEnd) { 
				if (phyRadioState.getRadioState() == RadioState.RECEIVING) {
					...
					if (!msgColision) {
						...
						colisão durante a recepção. */
						/* para que seja confirmada a recepção completa de uma mensagem, 
	...
	private double transmissionSteps(int packetLength){
		double stepsTx = ((double)packetLength / bpsSimulation) * getConfig().getStepsPerSecond();
		if (stepsTx < 0)
