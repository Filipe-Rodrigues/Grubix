...
import br.ufla.dcc.PingPong.XMac2019.XMacState.XMacEventType;
import br.ufla.dcc.PingPong.XMac2019.XMacState.XMacStateType;
...
public class XMac extends MACLayer {
	...
	public void initConfiguration(Configuration config) throws ConfigurationException {
		...
		// em zero.
		xState = new XMacState(XMacStateType.SLEEP, 0);

		// Inicializa as variáveis de configuração
		xConf = new XMacConfiguration(cycleTime, ackRequested);

		// Cria a instância da máquina de estados
		xStateMachine = new XMacStateMachine(sender, xState, xConf);
	}
	...
	public final void processWakeUpCall(WakeUpCall wuc) {
		...
		if (wuc instanceof CrossLayerEvent) {
			...
			// XMacPacket
			else if (!(cle.getPacket() instanceof XMacPacket)) {
				return;
			}
	...
	public final void lowerSAP(Packet packet) {
		...
		XMacPacket xPack = (XMacPacket) packet;
		...
		case RTS:
			event = XMacEventType.RTS_RECEIVED;
			break;
		case CTS:
			event = XMacEventType.CTS_RECEIVED;
			break;
		case ACK:
			event = XMacEventType.ACK_RECEIVED;
			break;
		case DATA:
			event = XMacEventType.DATA_RECEIVED;
			break;
		default:
			event = XMacEventType.VOID;
		}
	...
	private void gotoNextState(XMacEventType event) {
		...
		case START_RTS:
			...
			if (!startRTSsequence()) {
				...
				sendXMacPacket(XMacStateType.SENDING_RTS);
			}
			if (debug)
	...
	private boolean startSendDataProcess(Packet packet) {
		...
		XMacPacket newDATApacket = createDataPacket(llpack, xConf.isACKrequested());
	...
	private boolean startRTSsequence() {
		 ...
		 * RTS no XMacState.
		 */
		XMacPacket newRtsPacket = createCtrlPacket(xState.getReceiverNode(), PacketType.RTS, xConf.getMaxPreambles(),
	 ...
	 */
	private PacketType sendXMacPacket(XMacStateType stateType) {
	 ...
	 * e operação do rádio.
	 */
	XMacWucTimeOut createtWucTimeOut() {
	...
	private XMacPacket createCtrlPacket(NodeId receiver, PacketType type, int retryCount, boolean ACKreq,
			int headerLength, double signalStrength) {
		XMacPacket newPack = new XMacPacket(this.sender, receiver, type, signalStrength);
	...
	private XMacPacket createDataPacket(LogLinkPacket packet, boolean ACKreq) {
		XMacPacket newPacket = new XMacPacket(this.sender, packet, ACKreq);
		// Tamanho do pacote em bits
