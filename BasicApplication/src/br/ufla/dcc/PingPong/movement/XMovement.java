 ...
 *
 */
public class XMovement extends MovementManager {
       /**
...
// minhas adicoes
        ...
        * true: from the right to the left
        * false: from the left to the right
        * default: true
        */
...
// fim das minhas adicoes
       ...
       public void sendCommand (String command, NodeId id, double value1) {
               ...
               if (this.speeds!=null && command.contains("increase_speed")) {
                       this.speeds.put(id.asInt(),this.speeds.get(id.asInt()) + value1);
                       if (this.speeds.get(id.asInt())>1)    {
                               this.speeds.put(id.asInt(),1.0);
                       }
               }
       }
       ...
       public final Collection<Movement> createMoves(Collection<Node> allNodes) {
               ...
               if (this.speeds==null)
               {
                       ...
                       {
                               this.speeds.put(node.getId().asInt(),Math.random());
                       }
               }
               ...
               for (Node node : allNodes)
               {
                       ...
                       // Velocidade de 0 a 100 km por hora;
                       // 1 unidade de distancia = 50m
                       
                       double passo = (100.0/3.6 * Configuration.getInstance().getSeconds(configuration.getMovementTimeInterval()))/50.0;
                       double speed = speeds.get(node_id); //Speed of the movement
