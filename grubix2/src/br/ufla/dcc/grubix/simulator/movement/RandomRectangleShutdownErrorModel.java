...
/**
 * Makes all nodes positioned within a rectangle of a variable size to shutdown
 * for a specified time with a configurable probability. 
 * 
 * @author Thomas Kemmerich
 */
public class RandomRectangleShutdownErrorModel extends MovementManager {

	/** 
	 * Logger.
	 */
	private static final Logger LOGGER = Logger.getLogger(RandomRectangleShutdownErrorModel.class);
	...
	public Collection<Movement> createMoves(Collection<Node> allNodes) {
		...
		LinkedList<Node> allNodesLinkedList = new LinkedList<Node>(allNodes);
		// get the node that determines the top left position
		int randomValue = random.nextInt(allNodes.size());
		
		// determine the dimensions of the rectangle
		double width = random.nextDouble() * maxWidth;
		double heigth = random.nextDouble() * maxHeight;
				
		Position topLeftPosition = allNodesLinkedList.get(randomValue).getPosition();
