 ...
 * @author jlsx
 */
public class RandomStartDistantPositions extends StartPositionGenerator {
	...
	private Vector<Position> posicoes = new Vector<Position>(200);
	...
	public final Position newPosition(Node node) {
		...
		if(node.getId().asInt()==0){
			...
			do {
				 ...
				 y = random.nextDouble() * ySize;
				 y = Math.floor(y * 100) / 100;
			}while(distanceToNearst(x,y)<minimumDistance);
		}
